...
The narrow one: **a status of `completed` describes the request the server processed, which may not be the request you sent.** Here the server processed "what is my secret word?" and completed it perfectly. The `previous_response_id` was not rejected, not warned about, not `incomplete_details`-ed. It was not there by the time anything could have complained. The same server does it one layer down, too: its [gemma4 renderer drops any tool parameter named `type` or `description` from what the model sees](https://homelabpostmortem.com/2026/09/16/ollama-gemma4-drops-tool-parameters-named-type/) while still listing it as required, and the model fills the gap with a guess — same 200, same silence, a name dropped in the prompt instead of in the request.
